...
inline void gc_string_ensure(GC_String* str, uint add) {
   if (str->len + add + 1 <= str->cap) return;
   while (str->len + add + 1 > str->cap) str->cap *= 2;
   str->data = (char*)GC_realloc(str->data, str->cap);
}

inline void gc_string_append(GC_String* str, const char* append, uint append_len) {
...
inline void gc_string_appendf(GC_String* str, const char* fmt, double x) {
   char buf[64];
